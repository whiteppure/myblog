...
<div class="introduce_myself">
    ...
    <div class="myself_desc">
        <p>
           &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 现主要从事于Java工程师，就职于北京某公司，公司主要是为金融领域提供以客户为中心的应用软件开发。
           目前在江苏南京出差，从事Java开发工作经验为1年，有意向继续从事Java开发相关工作，期望工作地点在北京或上海、接受出差，工资面议。
        </p>
    </div>
</div>
...
- **职位**: Java实习生
- **简介**:
在校期间参与*直流远程供电后管系统*建设。
系统基于`springboot`进行开发、前后端分离开发。主要分为参数模块，设备模块等。
主要负责Java后端设备模块及主要模块开发。

#### 贵州银行”紫薇工程”-电子验印系统
- **时间**：2019.11～2020.11
- **职位**：初级Java开发工程师
- **简介**:
新电子验印系统基于验印系统原型进行升级、改造。新电子验印系统采用前后端分离设计，之后集成到客户端。后端基于`springboot`开发，前端基于`vue.js`进行开发，采用分布式部署。
在项目担任核心，主要负责电子验印系统前端开发、后管系统开发、第三方系统调用接口功能开发（柜面接口服务等） 、外系统对接（集中作业系统，影像平台系统，银企对账系统等）等，
以及相关维护，通过`easyOps`流水线持续发版更新等。

## 工作能力
- 熟练使用`SSM`,`Springboot`等主流框架技术;
- 熟练使用`CSS、Javascript、Jquery`等相关技术;
- 有使用`Redis、Vue`、以及对接第三方系统等相关经验;
- 熟悉`MySql、Oracle`基本操作,熟练使用`Sql`;
- 熟悉Java面向对象编程，有良好的编码习惯;
- 了解`Sql`优化、数据库引擎、JVM相关原理及设计模式;
- 了解`Docker`、`MQ`、`nginx`、`elesticserach`等相关技术;
- 掌握`Linux`基本操作,掌握自动化运维部署基本操作;
- 熟练使用`Maven`、`Gradle`管理依赖工具;`Git`、`Svn`版本控制工具;以及基本软件开发工具;
- 具有良好的编码习惯对代码有”洁癖”,有一定的文档编写能力;
- 具有良好的沟通表达能力,学习及领悟能力,较强的责任心与团队精神;
