...
## 并行、并发、串行
- 如果CPU多核，每个CPU核心上执行一个任务，当一个CPU核心执行一个进程时，另一个CPU核心可以执行另一个进程，两个进程互不抢占CPU资源，这些任务是同时执行的，这叫并行；
...
### 串行
在同一时间段内只允许有一个CPU用于执行程序，代码按照从上到下的顺序来执行，不允许同时进行。
...
上述引用垃圾回收的前提条件是：对象都是可触及的，如果对象不可触及就直接被垃圾回收器回收了。
### 强引用
