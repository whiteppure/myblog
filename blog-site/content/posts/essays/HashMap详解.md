...
```
为了更好的理解举个例子：假设h=2，或h=3，length=15，进行与运算，最终逻辑与运算后的结果是一致的；
...
————————————
  0000 1110
````
因为采用`&`运算来计算分到哪个位置，为了避免`length=15`这类问题出现，所以采用2的N次幂这种方式，
因为2的N次幂的结果减一都是转换为二进制后都是以`...1111`结尾的，所以在进行逻辑与运算时碰撞几率小。
