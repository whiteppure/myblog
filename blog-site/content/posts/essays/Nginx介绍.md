...
### 反向代理
![nginx反向代理](/myblog/posts/images/application/nginx反向代理.jpg)
...
务器，也就是我们所说的负载均衡。
![nginx负载均衡](/myblog/posts/images/application/nginx负载均衡.png)uig./rh/e,

### 动静分离
为了加快网站的解析速度，降低原来单个服务器的压力，可以把动态页面和静态页面由不同的服务器来解析。

![nginx动静分离](/myblog/posts/images/application/nginx动静分离.jpg)

## Docker安装nginx

如未安装docker移步[安装 docker](/myblog/posts/springboot/springboot-docker)
...
```
![nginx存放路径](/myblog/posts/images/application/nginx安装路径.jpg)
...
```
![nginx测试修改hosts文件](/myblog/posts/images/application/nginx测试修改hosts文件.jpg)


在本地 `~/Documents/config/nginx/www` 下新建 test 目录，并编写一个 `index.html` 测试文件
![nginx本地路径](/myblog/posts/images/application/nginx本地路径.jpg)
```
<!DOCTYPE html>
<html lang="en">
...
![测试nginx成功](/myblog/posts/images/application/测试nginx成功.jpg)
